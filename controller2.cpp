...
void Controller::handlePlatformRequest(std::vector<Event>::iterator cur) { //richiesta di un binario alla stazione successiva (che so già)
	...
	std::cout << "Il treno " << cur->GetTrain()->identifying_number << " richiede un binario alla stazione " << cur->GetStation()->st_name << std::endl;
	...
	else																					//Nel caso di una stazione locale
	{							
